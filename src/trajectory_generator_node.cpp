...
void trajGeneration(Eigen::MatrixXd path)
{
    ...
    _polyCoeff = _trajectoryGeneratorWaypoint.PolyQPGeneration(_dev_order, path, vel, acc, _polyTime);
    // std::cout << "polycoeff: " << _polyCoeff << std::endl;

    visWayPointPath(path);
    // for (int i = 0; i < path.rows() ; ++i) {
    //     std::cout << "path: " << i << " " << path(i, 0) << " " << path(i, 1) << " " << path(i, 2) << std::endl;
    // }
...
{        
    ...
    {   
        for (double t = 0.0; t < time(i); t += 0.01, count += 1)
        {
          ...
          cur(1) = pt.y = pos(1);
          cur(2) = pt.z = pos(2);
          // std::cout << "point: " << count << " " << cur(0) << " " << cur(1) << " " << cur(2) << std::endl;
    ...
    // std::cout << "traj size: " << _traj_vis.points.size() << std::endl;
...
Vector3d getPosPoly( MatrixXd polyCoeff, int k, double t )
{
    ...
    for ( int dim = 0; dim < 3; dim++ )
    {
        ...
        VectorXd coeff = (polyCoeff.col(dim)).segment( k * _poly_num1D, _poly_num1D );
        ...
        // std::cout << "dim:" << dim << " coeff:" << coeff <<std::endl;
    }
...
{ 
    ...
    double vel = _Vel;
    double acc = _Acc;
    double vel_inv = 1 / vel;
    double acc_inv = 1 / acc;

    double acc_time = vel * acc_inv;
    double acc_dist = 0.5 * acc * acc_time * acc_time;
    
    double t = 0.0;
    for (int i = 0; i < Path.rows() -1 ; ++i) {
        double dist = (Path.row(i+1) - Path.row(i)).norm();
        if (dist < 2 * acc_dist) {
            t = 2.0 * sqrt(dist * acc_inv);
        } else {
            t = 2 * acc_time + (dist - 2 * acc_dist) * vel_inv;
        }
        time(i) = t;
    }
